...
library(dplyr)
library(ggmap)
#library(VTrack) erro: there is no package called 'VTrack'
library(ggplot2)
library(basemaps)
library(ggOceanMaps)
library(sp)
...
coordinates(MB5371_Receivers_SPATIAL) <- c("receiver_deployment_longitude",
...
### Let's see how many different sharks and different species are in the data set 
###(Should be 4 !!), we can save those ID codes as an object for later
...
Switch to a dataset with lots of tiger sharks.
Import data
```{r}
MB5371_Tiger_Sharks <- read.csv("../data/MB5371_Tiger_Sharks.csv", header = TRUE)
MB5371_Tiger_Sharks_Meta <- read.csv("../data/MB5371_Metadata_Tigers.csv", header = TRUE)
```
...
Now, import data frame with daily tiger shark counts in the Whitsundays
```{r}
MB5371_Whitsundays_Temp <- read.csv("../data/MB5371_Whitsundays_Temp.csv", header = TRUE)
```
...
#Another way to plot this with ggarrange instead
Tiger<-ggplot(data=Combined,mapping=aes(x=Date,y=Count))+geom_line()
Temp<-ggplot(data=Combined, mapping=aes(x=Date,y=degree_C))+geom_line()

Temp_Tiger<- ggarrange(Tigers,Temp,
                       labels=c("A","B"),
                       ncol=1,nrow=2)
